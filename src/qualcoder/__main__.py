...
def gui():
    ...
    # When the user follows the system colour scheme, re-apply the matching
    # theme live as the OS switches between light and dark mode (Qt 6.5+).
    def _reapply_theme(_color_scheme=None):
        if settings.get('stylesheet') != 'auto':
            return
        app.setStyleSheet(qual_app.merge_settings_with_default_stylesheet(settings))
        qta.reset_cache()
        qta.set_defaults(
            color=qual_app.qtawesome_icon_color,
            color_disabled=qual_app.qtawesome_icon_color_disabled
    ...
    try:
        app.styleHints().colorSchemeChanged.connect(_reapply_theme)
    except AttributeError:
        # Older Qt without the colour-scheme hint; theme stays as initially resolved.
        pass
